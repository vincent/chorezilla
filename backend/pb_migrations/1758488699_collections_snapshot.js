  ...
  const snapshot = [
    {
      ...
      "indexes": [
        ...
        "CREATE INDEX `idx_NkKfTYD8qG` ON `users` (`username`)"
      ],
    ...
    },
    {
      ...
      "deleteRule": "id ?= @request.auth.households.id\n&& @collection.household_members.household.id  ?= id\n&& @collection.household_members.user.id ?= @request.auth.id\n&& @collection.household_members.role ?= \"admin\"",
      "fields": [
        {
          "autogeneratePattern": "[a-z0-9]{15}",
          "hidden": false,
      ...
      "id": "pbc_2123300356",
      "indexes": [],
      "listRule": "id ?= @collection.household_members.household.id && @collection.household_members.user.id ?= @request.auth.id",
      "name": "households",
      "system": false,
      "type": "base",
      "updateRule": "id ?= @request.auth.households.id\n&& @collection.household_members.household.id  ?= id\n&& @collection.household_members.user.id ?= @request.auth.id\n&& @collection.household_members.role ?= \"admin\"",
      "viewRule": "id ?= @collection.household_members.household.id && @collection.household_members.user.id ?= @request.auth.id"
    },
    {
      "createRule": "@request.auth.id != null",
      "deleteRule": "household.id ?= @request.auth.households.id\n&& @collection.household_members.household.id  ?= household.id\n&& @collection.household_members.user.id ?= @request.auth.id\n&& @collection.household_members.role ?= \"admin\"",
      "fields": [
        {
          "autogeneratePattern": "[a-z0-9]{15}",
          "hidden": false,
