	...
	>
		{#if show === 'intro'}
			<Card.Root class="mx-auto max-w-sm">
				...
				<Card.Footer>
					<Button class="mt-8 h-20 w-full text-xl" onclick={() => showPermissions()}
		...
		{:else}
			...
			{#if done}
				...
				<Button
					...
					class="mt-8 h-20 w-full text-xl"
					onclick={() => input?.click()}>Scan a spot to start a game</Button
				>
			{:else}
				<Button
					disabled={$geoLocationStatus !== 'granted'}
					class="mt-8 h-20 w-full text-xl"
					href={redirectUrl}>Done</Button
				>
			{/if}
		{/if}
	</div>
