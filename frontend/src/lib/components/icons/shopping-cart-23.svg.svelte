<script lang="ts">
	let { width = 64, height = 64 } = $props();
</script>
<svg height={`${height}px`} width={`${width}px`} version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
	 viewBox="0 0 512 512"  xml:space="preserve">
<style type="text/css">
	:root { color-scheme: dark light; }
</style>
<g>
	<polygon fill="currentColor" points="420.676,386.988 420.676,346.29 217.265,346.29 196.972,288.622 439.538,288.622 512,90.553 
		468.67,90.553 411.098,247.924 182.648,247.924 114.146,53.241 97.675,53.241 0,53.241 0,93.939 85.323,93.939 188.443,386.988 	
		"/>
	<circle fill="currentColor" cx="197.425" cy="442.607" r="32.558"/>
	<circle fill="currentColor" cx="409.054" cy="442.607" r="32.558"/>
	<path fill="currentColor" d="M286.468,198.331c0.898,0.89,1.86,1.678,2.869,2.345c3.052,2.011,6.566,3.02,10.079,3.02
		c4.69,0,9.372-1.788,12.949-5.366l58.328-58.328c7.154-7.154,7.154-18.743,0-25.897c-7.154-7.155-18.743-7.155-25.897,0
		l-27.066,27.066V55.149c0-10.119-8.203-18.314-18.314-18.314c-10.111,0-18.314,8.195-18.314,18.314v86.03l-27.074-27.073
		c-7.154-7.155-18.752-7.155-25.897,0c-7.154,7.154-7.154,18.743,0,25.897L286.468,198.331z"/>
</g>
</svg>