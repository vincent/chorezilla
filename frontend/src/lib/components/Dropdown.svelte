<script lang="ts">
	...
	const handleDropdownFocusLoss = ({ relatedTarget, currentTarget }: FocusEvent) => {
		...
		if (relatedTarget instanceof HTMLElement && (currentTarget as any)?.contains(relatedTarget)) return // check if the new focus target doesn't present in the dropdown tree (exclude ul\li padding area because relatedTarget, in this case, will be null) 
...
<div class="flex justify-between items-center">
	<div class="dropdown" onfocusout={handleDropdownFocusLoss}>
		...
		<ul class="dropdown-content menu p-2 shadow bg-base-100 rounded-box bg-white dark:bg-gray-900 w-52" style:visibility={isDropdownOpen ? 'visible' : 'hidden'}>
			{@render menu?.()}
		</ul>
	</div>
</div>
